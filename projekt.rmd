...
# anova test of model fitting and complexity

anova(linear_model, quadratic_model)
# using level of significance p = 0.05 and given p < 2.2e-16, we conclude that significant gain is achieved using quadratic model in lieu of linear model

# 95% confidence interval for parameters of quadratic model
...
confident <- data.frame(Tlak = fh_concat$Tlak, Vreliste = predict(quadratic_model, fh_concat, interval = 'confidence'))
...
predicted <- data.frame(Tlak = fh_concat$Tlak, Vreliste = predict(quadratic_model, fh_concat, interval = 'prediction'))
